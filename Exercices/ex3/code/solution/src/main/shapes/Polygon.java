...
// SOL Class implementing Polygon
public class Polygon extends AShape {
    ...
    public double area() {
    	// SOL Calculation of a polygon's area
        // Calculates the area of theis polygon based on Green's Theorem in a plane.
        // Remark that the result will be wrong in case of non-convex polygons...!
        double thearea  = 0;
        int length = this.x.length;
        for (int i=0; i < length-1; i++) {
            thearea += this.x[i]*this.y[i+1] - this.x[i+1]*this.y[i];
        }
        thearea += this.x[length-1]*this.y[0]-this.x[0]*this.y[length-1];
        return Math.abs(thearea/2);
    }
    ...
    public boolean contains(Point p) {
        ...
        //number of crosses left/right from the mouseclick x coordinate
