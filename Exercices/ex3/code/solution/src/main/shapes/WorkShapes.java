...
public class WorkShapes extends javax.swing.JFrame {
    ...
    private void sortIDItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_sortIDItemActionPerformed
        // SOL Sorting the shapes by ID
    	Collections.sort(shapes, new ByIDShapeComparator());
        refreshShapes();
    ...
    private void areaMenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_areaMenuItemActionPerformed
        // SOL Displaying the sum of all areas
    	double totar = 0;
        for (int i = 0; i < shapes.size(); i++) {
            totar += shapes.elementAt(i).area();
        }
        textzone.setText('\n' + "Sum of all areas = " + totar);
    ...
    private void sortPerimeterItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_sortPerimeterItemActionPerformed
        // SOL Sorting the shapes by perimeter
    	Collections.sort(shapes, new ByPerimeterShapeComparator());
        refreshShapes();
    ...
    private void sortAreaItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_sortAreaItemActionPerformed
        // SOL Sorting the shapes by area
    	Collections.sort(shapes, new ByAreaShapeComparator());
        refreshShapes();
    ...
    public void createNewCircle(int x, int y, int r) {
    	// SOL Creating a new circle
        shapes.add(new Circle( x , y , r , Color.green, IdGenerator.getUniqueInstance().nextId() ));
        refreshShapes();
    }
    
    public void createNewTriangle(int[] vx, int[] vy) {
        shapes.add(new Triangle( vx , vy , Color.orange, IdGenerator.getUniqueInstance().nextId() ));
        refreshShapes();
    }
    ...
    public void createNewEllipse(int x, int y, int a, int b) {
    	// SOL Creating a new ellipse
        shapes.add(new Ellipse( x , y , a, b, Color.red , IdGenerator.getUniqueInstance().nextId() ));
        refreshShapes();
    }
    ...
    public void moveShape(int id, int dx, int dy) {
    	// SOL Moving a shape
        findShape(id).move(dx, dy);
        refreshShapes();
    }
    
    public void moveAllShapes(int dx, int dy) {
    	// SOL Moving all shapes
        for (int i = 0; i < shapes.size(); i++){
            shapes.elementAt(i).move(dx, dy);
        }
        refreshShapes();
    }
    
    public void deleteShape(int id) {
    	// SOL Deleting a shape
        shapes.removeElement(findShape(id));
        refreshShapes();
    }
    
    public void openShapes(String filename) {
    	// SOL Opening a file containing shapes
        try {
            FileInputStream fis = new FileInputStream(filename);
            ObjectInputStream ois = new ObjectInputStream(fis);
            shapes.clear();
            shapes.addAll((Vector<Shape>) ois.readObject());
            ois.close();
        }
        catch (Exception e) {
        ...
        // SOL Managing the shape IDs after opening a file
        // Set the last id of the IDGenerator correctly...
        int maxId = 0;
        for (int i = 0; i < shapes.size(); i++) {
            if (shapes.elementAt(i).getID() > maxId) {
                maxId = shapes.elementAt(i).getID();
            }
        }
        IdGenerator.getUniqueInstance().setLastId(maxId);
        refreshShapes();
    }
    
    public void saveShapes(String filename) {
    	// SOL Saving the shapes to a file
        try {
